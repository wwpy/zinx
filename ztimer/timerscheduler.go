...
/**
*  返回一个定时器调度器 ，主要创建分层定时器，并做关联，并依次启动
 */
	...
	go func() {
		for {
			...
			for tID, timer := range timerList {
				if math.Abs(float64(now-timer.unix_ts)) > MaxTimeDelay {
					// 已经超时的定时器，报警
					zlog.Error("want call at ", timer.unix_ts, "; real call at", now, "; delay ", now-timer.unix_ts)
				}
				if ts.HasTimer(tID) {
					// 将超时触发函数写入管道
