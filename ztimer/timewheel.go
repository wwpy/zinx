...
func NewTimeWheel(name string, interval int64, scales int, maxCap int) *TimeWheel {
	...
	zlog.Info("Init timer wheel name = ", tw.name, " is Done!")
...
* 当前timer的超时时间间隔 大于一个刻度，那么进行hash计算 找到对应的刻度上添加
* 当前的timer的超时时间间隔 小于一个刻度
* 没有下一轮时间轮
 */
func (tw *TimeWheel) addTimer(tID uint32, t *Timer, forceNext bool) error {
	defer func() error {
		if err := recover(); err != nil {
			err_str := fmt.Sprintf("addTimer function err : %s", err)
			zlog.Error(err_str)
	...
	delayInterval := t.unix_ts - UnixMilli()
...
func (tw *TimeWheel) AddTimeWheel(next *TimeWheel) {
	...
	zlog.Info("Add timerWheel[", tw.name, "]'s next [", next.name, "] is success!")
}
...
func (tw *TimeWheel) Run() {
	go tw.run()
	zlog.Info("timer wheel name = ", tw.name, " is running...")
}
...
func (tw *TimeWheel) GetTimerWithIn(duration time.Duration) map[uint32]*Timer {
	...
	// 找到最底层时间轮
	leaf_tw := tw
	for leaf_tw.nextTimeWheel != nil {
		leaf_tw = leaf_tw.nextTimeWheel
	}

	leaf_tw.Lock()
	defer leaf_tw.Unlock()
	// 返回的Timer集合
	...
	// 取出当前时间轮刻度内全部Timer
	for tID, timer := range leaf_tw.timerQueue[leaf_tw.curIndex] {
		if timer.unix_ts-now < int64(duration/1e6) {
			// 当前定时器已经超时
			timerList[tID] = timer
			// 定时器已经超时被取走，从当前时间轮上 摘除该定时器
			delete(leaf_tw.timerQueue[leaf_tw.curIndex], tID)
		}
	}
